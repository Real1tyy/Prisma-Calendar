...
describe("FilterEvaluator", () => {
	...
	describe("Basic filtering", () => {
		...
		it("should filter by inequality", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Status !== 'Inbox'"],
			});
		...
		it("should handle array properties", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Array.isArray(Project) && Project.length > 0"],
			});
		...
		it("should handle string operations", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Title && Title.includes('Meeting')"],
			});
	...
	describe("Complex filtering", () => {
		it("should handle OR conditions", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Status === 'Done' || Status === 'In Progress'"],
			});
		...
		it("should handle AND conditions across multiple filters", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Status !== 'Inbox'", "Priority === 'High'"],
			});
		...
		it("should handle nested property access", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Goal && Goal[0] && Goal[0].includes('Clarity')"],
			});
		...
		it("should handle complex logical expressions", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["(Status === 'Done' || Status === 'In Progress') && Priority !== 'Low'"],
			});
		...
		it("should handle date comparisons", () => {
			...
			settingsStore.next({
				...mockSettings,
				filterExpressions: [`Start_Date >= '${today}'`],
			});
	...
	describe("Error handling", () => {
		it("should ignore invalid expressions", () => {
			...
			evaluator.evaluateFilters({ Status: "Done", Priority: "High" });
		...
		it("should handle runtime errors in expressions", () => {
			...
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Status && Status.nonExistentMethod()"],
			});
		...
		it("should treat non-boolean results as false", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Status"], // returns string, not boolean
			});
		...
		it("should handle expressions that throw during evaluation", () => {
			...
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Status ? JSON.parse('{invalid json}') : true"],
			});
	...
	describe("Real-world examples", () => {
		...
		it("should handle multiple complex filters", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Status !== 'Inbox'", "!_Archived", "Priority === 'High' || Difficulty <= 2"],
			});
	...
	describe("Performance and edge cases", () => {
		it("should handle large numbers of filters efficiently", () => {
			const manyFilters = Array.from({ length: 100 }, (_, i) => `prop${i} === 'value${i}'`);
		...
		it("should handle rapid settings changes", () => {
			for (let i = 0; i < 10; i++) {
				settingsStore.next({
					...mockSettings,
					filterExpressions: [`Status === 'Status${i}'`],
				});
			}
		...
		it("should handle special characters in property names", () => {
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["Start_Date === '2024-01-01'", "_ZettelID > 0"],
			});
		...
		it("should handle deeply nested object access safely", () => {
			...
			settingsStore.next({
				...mockSettings,
				filterExpressions: ["deep && deep.nested && deep.nested.property === 'value'"],
			});
