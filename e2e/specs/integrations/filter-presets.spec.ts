...
const FILTER_PRESET_SELECT = '[data-testid="prisma-fc-filter-preset-select"]';
...
test.describe("filter presets", () => {
	test.beforeEach(async ({ obsidian }) => {
		await updateCalendarSettings(obsidian.page, { filterPresets: INITIAL_PRESETS });
	});

	test("CRUD via settingsStore persists to data.json", async ({ obsidian }) => {
		const afterCreate = await readCalendarSettings(obsidian.page);
		...
		const afterEdit = await readCalendarSettings(obsidian.page);
		...
		const afterDelete = await readCalendarSettings(obsidian.page);
		expect((afterDelete["filterPresets"] as unknown[]).length).toBe(1);
	});

	test("open-filter-preset-selector command exposes every preset", async ({ obsidian }) => {
		await openCalendar(obsidian.page);
		const select = obsidian.page.locator(FILTER_PRESET_SELECT).first();
		await select.waitFor({ state: "attached" });

		await runCommand(obsidian.page, "Prisma Calendar: Open filter preset selector");
