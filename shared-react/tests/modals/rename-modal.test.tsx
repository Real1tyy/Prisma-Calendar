...
describe("RenameModalContent", () => {
	...
	it("renders with initial value in the input", () => {
		renderInTheme(<RenameModalContent {...defaultProps} />);
	...
	it("fires onSubmit with trimmed value when Save clicked", async () => {
		...
		const { user } = renderInTheme(<RenameModalContent {...defaultProps} onSubmit={onSubmit} />);
	...
	it("fires onCancel when Cancel clicked", async () => {
		...
		const { user } = renderInTheme(<RenameModalContent {...defaultProps} onCancel={onCancel} />);
	...
	it("renders a description above the input when provided", () => {
		renderInTheme(<RenameModalContent {...defaultProps} description="Renaming will affect 5 items." />);

		expect(screen.getByText("Renaming will affect 5 items.")).toBeInTheDocument();
	});

	it("renders extras slot between input and footer", () => {
		renderInTheme(<RenameModalContent {...defaultProps} extras={<div data-testid="extra-slot">extra content</div>} />);

		expect(screen.getByTestId("extra-slot")).toBeInTheDocument();
	});

	it("extras slot is omitted when not provided", () => {
		renderInTheme(<RenameModalContent initialValue="x" onSubmit={noop} onCancel={noop} />);
	...
	it("ignores cancel after submit (settled guard)", async () => {
		const onSubmit = vi.fn();
		const onCancel = vi.fn();
		const { user } = renderInTheme(<RenameModalContent {...defaultProps} onSubmit={onSubmit} onCancel={onCancel} />);
