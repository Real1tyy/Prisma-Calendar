 ...
 * which include DAV:valid-sync-token), so the sync service can clear the
 * cursor and fall back to a full refetch instead of surfacing the error to
 * the user.
 *
 * Extracted into its own module for two reasons:
 *   1. The logic is a pure check — no Obsidian or tsdav dependencies — so it
 *      can be unit-tested without spinning up the full sync service.
 ...
 *
 *   • RFC 6578 §3.2 — the server returns the <DAV:valid-sync-token/>
 ...
 *
 * The HTTP-410 regex is anchored behind a "status" / "HTTP" / "response"
 * prefix so a bare "410" inside an unrelated URL, timestamp, or payload body
 * does NOT trip the fallback. Benign mismatches are fail-safe (one extra full
 * refetch, never data loss), but the previous broader catch-all could mask
 * real server errors.
 */
export const TOKEN_INVALIDATED_PATTERNS: readonly RegExp[] = [
	/valid[-_ ]sync[-_ ]token/i,
	/(?:\bstatus(?:[_ ]?code)?|\bHTTP(?:\/\d+(?:\.\d+)?)?|\bresponse(?:[_ ]?status)?)[\s:=-]*410\b/i,
];

export function isSyncTokenInvalidated(error: unknown): boolean {
	if (!error) return false;

	// Prefer a structured status field when the fetch / tsdav layer has
	// attached one — it's a stronger signal than regexing the message and
	// won't false-trip on `410` occurring inside the textual body.
