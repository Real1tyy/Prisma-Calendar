...
- **Page header actions cleanup**: Removed actions that don't work well as standalone buttons (batch operations, focused event actions, focus/filter commands, toggle untracked, toggle prerequisites, timeline, heatmap). Added "Show Interval in Bases" to defaults. See [Page Header Actions](./features/calendar/calendar-view.md).
