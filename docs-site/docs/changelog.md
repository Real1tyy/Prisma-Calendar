...
- **Prerequisite property**: New "Prerequisite" frontmatter property for declaring chronological dependencies between events. Set prerequisites as comma-separated wiki-links (e.g. `[[Event A]], [[Event B]]`) in the event create/edit modal. Configurable property name in Settings → Properties. See [Properties](./configuration/properties.md).
