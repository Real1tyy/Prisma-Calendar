...
- **Edit modal using stale start/end dates**: The edit event modal loaded start and end times from the in-memory calendar data instead of the actual frontmatter on disk. If the calendar's cached dates diverged from the file (e.g., after "Fill end time from next event"), editing any field and saving would overwrite the correct frontmatter values with the stale ones. The modal now reads start/end times directly from frontmatter when opening.
