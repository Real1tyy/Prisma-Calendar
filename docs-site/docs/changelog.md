...
- **Event name typo detection**: When creating or editing an event, the plugin now uses fuzzy matching to detect likely typos in event names. If the entered name closely matches a known category, preset event name, or existing event series, an inline "Did you mean?" suggestion appears below the title field. Clicking the suggestion corrects the title and auto-assigns the matching category. This feature can be toggled off in the category auto-assignment settings. See the [Configuration](./configuration.md#auto-assign-categories) documentation.
