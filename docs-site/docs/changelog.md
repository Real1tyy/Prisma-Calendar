...
> **TLDR:** Jump the calendar straight to any date — click the toolbar title or run the new **Go to date** command instead of clicking the arrows week by week. Dragging an event into a different week now lands it exactly where you drop it — no more landing hours off the cursor or snapping back — and the all-day strip accepts drops anywhere in its height. Event text color is now chosen by real contrast — bright backgrounds like yellow get dark text instead of unreadable white — with a new nine-step **Text contrast sensitivity** setting controlling how eagerly the alternative color kicks in. Prerequisite arrows also stay anchored to the right event after you drag an event into a day's all-day lane.
...
- **Prerequisite arrows on all-day events**: dragging an event onto a day that holds several all-day events no longer leaves its arrow pointing at whichever event sits at the top of that day's stack — the arrow now follows the moved event to its final spot in the lane, with no need to toggle the arrows off and on to fix it, and without the momentary flicker to the top event while the day settles. Arrows also stop guessing when there's nothing to point at: an event collapsed behind a **+N more** link keeps its arrow hidden until the event is visible again, and while you drag an event its arrow tracks the drag preview. And any arrow touching the all-day row — between two all-day events, or from an all-day event to a timed one — now stays visible above the pinned all-day row instead of being swallowed by it. See [Prerequisite Connections](./features/advanced/prerequisite-connections.md#connection-arrows).
