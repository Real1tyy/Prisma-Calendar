...
> **TLDR:** New **series shortcut** context menu items let you jump directly to the name, category, or recurring tab in the Event Series Modal. New **Move to planning system…** context menu item moves an event between calendars and translates its frontmatter to the destination's schema. Settings now show an **Update available** badge next to the version when a newer Prisma Calendar release is published. Timeline, Heatmap, and Gantt views now react to settings and color changes in real time. **Daily + Stats**, **Monthly + Stats**, and **Heatmap Monthly + Stats** keep pie charts and totals live as vault events change. Settings now remember your active tab and scroll position. Icons now have a **visual picker** with live previews, are **optional on all UI elements**, **tabs support inline icons**, and the **tab manager** now lets you edit icons and colors per tab. **Categories now count untracked events** — they show up with proper totals and rename/delete optionally cascade to untracked notes, and **category rename / delete are now undoable** via the standard Undo/Redo commands. Two planning systems on the same directory with mismatched sort settings no longer fight each other.
