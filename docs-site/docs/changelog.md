...
- **Prerequisite arrows on all-day events**: an arrow no longer jumps to a different event when you reschedule onto a day with all-day events, and an arrow between two all-day events is no longer hidden behind the sticky all-day row. See [Prerequisite Connections](./features/advanced/prerequisite-connections.md#connection-arrows).
