...
> **TLDR:** The view-header action buttons are now responsive at every pane size — desktop and mobile alike — showing only as many icons as fit, tucking any that don't into a **More actions** overflow menu so they stay one click away, keeping **Manage** anchored at the top-right, and re-packing live as you resize the pane or reorder them. Plus a mobile / narrow-pane layout pass: the view tabs and view toolbars wrap so the **Search events…** box no longer disappears, the settings editors and the Create Event modal reflow so their fields stay usable, the split / dashboard views scroll to reveal the full distribution chart (legend hidden so the pie is visible), the yearly heatmap scrolls sideways, and drag-to-edge paging works with touch.
