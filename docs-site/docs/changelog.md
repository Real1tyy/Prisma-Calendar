...
> **TLDR:** Jump the calendar straight to any date — click the toolbar title or run the new **Go to date** command instead of clicking the arrows week by week. Dragging an event into a different week now lands it exactly where you drop it — no more landing hours off the cursor or snapping back — and the all-day strip accepts drops anywhere in its height. Event text color is now chosen by real contrast, so bright backgrounds like yellow or cyan get dark text instead of unreadable white.
