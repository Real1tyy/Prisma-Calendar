...
- **Programmatic calendar info and statistics**: New `getCalendarInfo`, `listCalendars`, and `refreshCalendar` API methods expose calendar metadata. `getStatistics` provides interval-based time breakdowns aggregated by event name or category — specify a date, interval (`day`/`week`/`month`), and aggregation mode to get duration totals, formatted times, percentages, and per-entry breakdowns. URL-accessible. See [Programmatic API — Calendar Info](./features/advanced/programmatic-api.md#calendar-info) and [Statistics](./features/advanced/programmatic-api.md#statistics).
- **Programmatic settings access**: New `getSettings` and `updateSettings` API methods for reading and modifying calendar settings programmatically. `updateSettings` shallow-merges partial settings into the current configuration and triggers reactive updates. Window-API-only. See [Programmatic API — Settings](./features/advanced/programmatic-api.md#settings).
- **AI conversation persistence**: AI chat conversations are now saved automatically and persist across panel closes and Obsidian restarts. A collapsible thread list lets you browse, search, and switch between past conversations. Each thread remembers its mode (Query, Manipulate, Plan). Thread titles are AI-generated summaries of the first message. See [AI Chat — Conversation History](./features/advanced/ai-chat.md#conversation-history).
- **AI category and event name context**: The AI chat now receives your full category list and category assignment presets, so it uses your existing categories and event naming conventions instead of inventing new ones. Configure category assignment presets in Settings > Categories for improved accuracy. See [AI Chat — Category & Event Name Context](./features/advanced/ai-chat.md#category--event-name-context).
...
- **Programmatic API on `window.PrismaCalendar`**: Added scripting API for creating events, creating untracked events, converting existing files to Prisma format, opening creation/edit modals, and enforcing ZettelID from external scripts/plugins. See the [Programmatic API documentation](./features/advanced/programmatic-api.md) for full details.
