...
> **TLDR:** Jump the calendar straight to any date — click the toolbar title or run the new **Go to date** command instead of clicking the arrows week by week. Imported and synced events now pick up your categories automatically, so a "Gym" event from Google Calendar lands under "Health" without you touching it. Dragging an event into a different week now lands it exactly where you drop it, and event text color follows real contrast — with a new nine-step **Text contrast sensitivity** setting controlling how eagerly the alternative color kicks in.
