...
- **Setup finds the date property you actually use**: choosing **Use notes you already have** in the Welcome modal (or **Configure calendar**) now offers your real all-day property instead of one of Prisma's own recurrence fields. A property named like a Prisma default — `Date`, `Start Date`, `End Date` — is offered first even when another dated property appears in more notes; properties Prisma writes itself (`RRuleUntil`, `Sort Date`, the recurrence fields) are excluded from the scan entirely, and vault bookkeeping like `Created` or `Modified` is offered last. The scan also reads dates however Obsidian stores them — plain text, a **Date** or **Date & time** typed property, or a value inside a list property — so a date-typed `Date` property is no longer skipped as if the folder had none. See [Quick Start → Adapting Prisma to a folder you already have](./quickstart.md#adapting-prisma-to-a-folder-you-already-have).
