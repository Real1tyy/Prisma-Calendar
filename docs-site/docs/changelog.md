...
- **Note content in event modals**: create and edit event modals include a full-width plain-text **Note content** field that expands to fit its text without an inner scrollbar, so agendas, meeting notes, and links can be saved without opening the note separately. The fixed event header, Title field, and divider, together with up/down arrow controls, keep the modal's scrolling form navigable. The field follows external note edits while it is untouched, warns before overwriting a concurrent edit, participates in undo, and is reachable with the header button or `Ctrl/Cmd+Shift+N`.
- **Event time display modes**: Choose **Normal**, **Small**, **Hidden**, or **Auto** separately for timed events in week/day and month/year views. Auto responds to the rendered tile: time drops from short or narrow tiles and returns as the tile has room, while phone month views keep their compact title-only layout. Configure both under **Settings → Calendar → Event appearance**. See [Calendar Settings](./configuration/calendar-ui.md#visual-appearance).
- **Privacy & diagnostics** documentation — a new docs page explains what stays on your device, what leaves it and when, and how any diagnostics you share are redacted first: paths and note names abbreviated, property values reduced to their type, secrets never included, and nothing sent without your explicit action.
...
### Improved
- The Properties tab now explains property ordering with worked before/after examples, spells out that your own keys keep their values and their relative order, and shows how the keys above the first Prisma property decide where the block lands.
- Every worked-example and explanation panel in settings folds away once you've read it — property order, event types, recurring events, display properties, and the four example panels on the Rules tab. Each remembers its own state on that device.
- **Settings explain multi-device use**: a **Using several devices** box under General (next to Read-only mode) and a **Syncing from several devices** box under Integrations describe what Prisma writes on its own, what to sync, and the situations that still call for a single writing device — auto-assigned ZettelIDs, reminders and the time tracker. The Read-only mode description lists everything it turns off. See [Multiple devices and sync → What to be aware of](./features/advanced/multi-device-sync.md#what-to-be-aware-of).
- **External calendar notes converge across devices**: CalDAV sync, ICS subscriptions, and manual ICS imports use stable UID-derived note names by default, repeated imports skip existing events, and subscriptions adopt matching imported notes. A **Synced note naming** setting keeps creation-time filenames available. See [Integrations](./features/advanced/integrations.md#several-devices).
...
### Added
- **Configurable property order**: the Properties settings tab is now a single reorderable table — rename any property as before, and drag rows (or use the arrow buttons) to choose the exact order Prisma writes to disk, with the most important scheduling properties first by default. Every save regroups Prisma's own properties into that order and leaves all other properties untouched, so synced devices produce byte-identical files instead of endless conflicts on keys like `Sort Date`. Files adopt the order the next time Prisma writes to them. The per-view display lists now sit together under a **Display properties** heading at the bottom of the tab. See [Properties Settings → Property Order](./configuration/properties.md#property-order).
- **Normalize property order**: a **Scan and normalize…** button on the Properties tab (also a command palette entry) scans every event file, lists the ones whose Prisma properties are out of the configured order (the first 20, plus a count of the rest), and — once you confirm — rewrites them in one pass with a progress bar and a completion summary. Files already in order are never touched. Run it on one device after changing the order and let sync propagate. See [Properties Settings → Normalize property order](./configuration/properties.md#normalize-property-order).
...
### Fixed
- **Pro feature notices are clickable**: the notice shown when you run a Pro command on the free plan pasted two long tracking URLs as plain text — unclickable, and impossible to copy out of a notice. It now shows **Docs** and **Get Pro** as real links you can click, and stays up for ten seconds so there's time to reach them. See [Free vs Pro](./features/free-vs-pro.md).
- **Setup finds the date property you actually use**: choosing **Use notes you already have** in the Welcome modal (or **Configure calendar**) now offers your real all-day property instead of one of Prisma's own recurrence fields. The names that planning system already uses — plus Prisma's defaults `Date`, `Start Date` and `End Date` — are offered first even when another dated property appears in more notes; properties Prisma writes itself (`RRuleUntil`, `Sort Date`, the recurrence fields) are excluded from the scan entirely, and vault bookkeeping like `Created` or `Modified` is offered last. Renaming any of these under **Properties** is respected, so re-running **Configure calendar** on a vault that uses your own property names ranks them correctly. See [Quick Start → Adapting Prisma to a folder you already have](./quickstart.md#adapting-prisma-to-a-folder-you-already-have).
- **Settings search filters again**: with Obsidian 1.13's **Open settings in new window** enabled, typing in the settings search box hid nothing — every section stayed on screen, even below the "No settings found" message. Search filters correctly again, and it now filters the whole page rather than only the individual setting rows: explanation boxes, color rules, presets, category rows and help cards answer to the query too, and a section disappears entirely when nothing in it matches. A query that matches a section heading shows that whole section — searching `color` keeps the rules under **Event colors** even though the rules themselves don't contain the word. See [Configuration → Settings Search](./configuration/index.md#settings-search).
...
- **Daily recurrence saves as shown**: creating an event with **Recurring Event** enabled and the **Recurrence pattern** dropdown left on **Daily** now writes `RRule: daily` and generates the instances, instead of saving an empty `RRule` and no instances. See [Recurring Events → Recurrence Patterns](./features/events/recurring-dsl.md#recurrence-patterns).
- **Readable +N more popover**: clicking a day's **+N more** link in Month or Year view opens a popover that uses the height your screen actually has — it runs from the day cell down to roughly 30px above the bottom of the window (pulling itself upward when the day sits low on screen) and scrolls inside if the day holds more than fits. The events listed in it no longer overlap each other either: each one gets its own row, sized to its title and property pills, so a crowded day reads cleanly instead of stacking text on top of text. See [Calendar View → Day Overflow Popover](./features/calendar/calendar-view.md#day-overflow-popover).
- **Prerequisite arrows on all-day events**: dragging an event onto a day that already holds several all-day events no longer leaves its arrow pointing at whichever event sits at the top of that day's stack — the arrow follows the moved event to its final spot in the lane, with no need to toggle the arrows off and on to fix it and no flicker while the day settles. Arrows also stop pointing at events you can't see: one collapsed behind a **+N more** link shows no arrow until it's visible again, and a dragged event's arrow tracks the drag preview. And an arrow touching the all-day row now stays visible above it instead of being swallowed when the row is pinned. See [Prerequisite Connections](./features/advanced/prerequisite-connections.md#connection-arrows).

## 2.22.0 - 8/4/2026

> **TLDR:** Jump the calendar straight to any date — click the toolbar title or run the new **Go to date** command instead of clicking the arrows week by week. Dragging an event into a different week now lands it exactly where you drop it — no more landing hours off the cursor or snapping back — and the all-day strip accepts drops anywhere in its height. Event text color is now chosen by real contrast — bright backgrounds like yellow get dark text instead of unreadable white — with a new nine-step **Text contrast sensitivity** setting controlling how eagerly the alternative color kicks in.
...
### Fixed
- **Cross-week drag lands under the cursor**: dragging an event to the screen edge to page into another week no longer drops it hours away from where you release it — or snaps it back to its original slot — even when the destination week shows a different number of all-day events. All-day events now drop reliably anywhere within a day's all-day strip (not just at its top edge), and the strip keeps a comfortable minimum height so it's always an easy target. While a drag is in flight the strip holds a steady height rendered as normal bordered day cells — arriving in a week with fewer all-day events no longer leaves a detached empty band below them — and the held event stays visible under your cursor across the whole strip. See [Calendar View → Scrolling while dragging](./features/calendar/calendar-view.md#scrolling-while-dragging).
- **Readable text on bright event colors**: events with vivid light backgrounds — yellow, cyan, light green — now use the alternative (dark) text color instead of near-invisible white, and named CSS colors in color rules (like `yellow`) are recognized too. Your default text color stays in charge everywhere it's readable: it only yields when its contrast against the event background drops below a readability floor, so mid-tone backgrounds (reds, oranges, medium greens) keep the look you're used to. The new **Text contrast sensitivity** setting (Settings → Calendar → Event text colors) picks the floor on a nine-step ladder — from **Always primary color** (never switch) through half-step floors (1.5:1 up to 4.5:1; default **Moderate**, 2.5:1) to **Maximum contrast** (always the stronger of the two) — so if, say, your greens switched to dark text and you wanted them white, drop a step or two and they come back. See [Calendar View → Event Text Coloring](./features/calendar/calendar-view.md#event-text-coloring).
...
### Improved
- **Responsive header actions at every pane size**: the view-header icon row now adapts to the available width on desktop and mobile alike — it shows only as many action buttons as fit, tucks the rest into a **More actions** (⋯) overflow menu at the end of the row so a trimmed action is always one click away (never stranded off-screen), keeps the **Manage** button anchored at the top-right so it's always reachable, and re-packs live as you resize the pane or reorder, add, or remove actions (an action you move into range appears immediately). See [Header Actions → How it works](./features/views/header-actions.md#how-it-works).
- **Clearer onboarding from settings**: the more involved settings now link straight to their documentation so help is one click away. A new [License & activation](./configuration/license.md) guide explains where your license key comes from, how it's stored, and how to activate Pro on each device — linked right from the License settings. The CalDAV and ICS URL subscription headings link to their guides, with a dedicated step-by-step [Connect Google Calendar](./features/advanced/integrations/google-calendar.md) guide for the most common setup.

### Fixed
- **Mobile**: a layout pass for phones and narrow panes. The view tabs wrap onto multiple rows so every tab (including Dual Daily and Dashboard) stays tappable instead of being cut off the right edge; the Timeline, Heatmap, and Gantt toolbars wrap so the filter and **Search events…** box stay visible instead of vanishing off the edge; the colour-rule, filter-preset, and category settings editors reflow onto two rows so their inputs are wide enough to type in; the Create Event modal centers its title with the **Virtual** toggle inline, keeps each checkbox and toggle on one line with its label, and stops the date picker from being clipped by compact **Now** / **Fill** buttons; the split and dashboard views (Dual Daily, Daily + Stats, Monthly + Stats, Dashboard) render their panels in a single scrolling column and scroll to reveal the full distribution chart, whose legend is hidden on phones so the pie itself stays visible; the yearly heatmap keeps its full width and scrolls sideways from January; and dragging an event to the left/right edge to page the calendar now works with touch, not just a mouse. See [Calendar View → Mobile layout](./features/calendar/calendar-view.md#mobile-layout).
...
### Fixed
- **Minimized stopwatch session is more robust**: While tracking, the End stayed pinned at the start+5-minute stamp instead of moving with the timer, and restoring a long-minimized session reopened with that outdated End — it now advances to the current time every 5 minutes while running, both in the open modal and on restore. Renaming the tracked note while a minimized session runs no longer drops the timer as if the event were deleted; tracking now follows the note to its new name. See [Time Tracker → Minimize & Auto-Save](./features/management/time-tracker.md#minimize--auto-save).
...
- **Reset to defaults in every Manage X modal**: The header action manager, tab manager, context menu item manager, and dashboard layout editor each have a **Reset to defaults** button at the top, next to the "Show settings button" toggle (or the dimension controls in the layout editor). Click it and confirm to wipe order, visibility, renames, icon overrides, and color overrides back to the shipped defaults. See [Header Actions → Reset to Defaults](./features/views/header-actions.md#reset-to-defaults), [Tabbed Views → Managing Tabs](./features/views/tabbed-views.md#managing-tabs), [Context Menu → Managing menu items](./configuration/toolbar-and-menus.md#managing-menu-items), and [Dashboard → Layout](./features/views/dashboard.md#layout).
...
- **Update available badge in settings**: When a newer release of Prisma Calendar is published, settings now show an **Update available** pill next to the version badge. Click it to jump to the GitHub release notes. The check runs once a day and can be turned off under **General → Check for updates**. See [General Settings → Check for updates](./configuration/general.md#check-for-updates).
- **Resizable split for Dual Daily, Daily + Stats, Monthly + Stats, and Heatmap Monthly + Stats**: Drag the divider between columns on any of these tabs to adjust the split. The position persists per calendar, so each tab opens at the size you left it. See [Dual Daily](./features/views/dual-daily.md), [Daily + Stats](./features/views/daily-stats.md), [Monthly + Stats](./features/views/monthly-calendar-stats.md), and [Heatmap Monthly + Stats](./features/views/heatmap-monthly-stats.md).

### Improved
- **Plugin bundle is ~22% smaller (5.5 MB → 4.4 MB)**: Pro feature preview screenshots now load from the docs site instead of being inlined into `main.js`, plus tighter tree-shaking of Chart.js and removal of an unused dependency. The plugin now syncs between devices on **Obsidian Sync's Standard plan** (5 MB per-file limit).
...
- **Categories count untracked events**: Categories now include untracked notes, so they now appear with real statistics totals, and rename / delete operations show an **"Also apply to N untracked events"** toggle (ticked by default) so the cascade reaches every note carrying the category. See [Categories → Managing Categories](./features/organization/categories.md#managing-categories).
- **Undo / redo for category rename and delete**: Both operations are now part of the standard undo history. After a rename or delete, the success notice exposes an inline **Undo** link, and **Prisma Calendar: Undo** / **Prisma Calendar: Redo** from the command palette walk the file rewrites plus the matching color rule in one step. See [Categories → Renaming and Deleting Categories](./features/organization/categories.md#renaming-and-deleting-categories).

### Fixed
- **Event modal accepted submits with no title or date**: Pressing Save on a fresh / cleared modal silently wrote an empty untracked event. The modal now requires a title plus a start time (or a date for all-day events) before letting you save.
- **CalDAV initial sync not fetching events**: First-time CalDAV sync sometimes produced no events, the initial sync now correctly fetches all calendar objects from the server. See [CalDAV Integration](./features/advanced/integrations.md#caldav-integration).
- **ICS export failing on events with reminders**: Exported ICS files with VALARM triggers used an invalid duration format or certain Rrule properties, causing re-import to fail with a parsing error. See [ICS Export](./features/advanced/integrations.md#exporting-events).
- **ICS import crashing on recurring-instance files**: Importing an exported `.ics` could fail with `Could not extract integer from ":1"`, blocking every event from being imported. Fixed, also the importer now skips any unreadable event with a clear in-modal warning instead of aborting the whole file. See [ICS Import](./features/advanced/integrations.md#importing-events).
...
- **Year view**: New **Year** toolbar button in the main calendar tab shows all twelve months in a compact grid. Events render as compact chips under "+more" button, color dots show at the top of each day cell (same as monthly view), and the entire palette matches your Obsidian theme. Click an event to open it, click a day to create one, use prev/next to step by year. See [Calendar View](./features/calendar/calendar-view.md#view-modes).
...
### Fixed
- **Events in subdirectories not appearing on the calendar**: Files stored more than one level deep under the configured calendar folder (e.g., `courses/CS101/assignments/HW1.md`) were silently excluded after v2.13.0. See [General Settings](./configuration/general.md#calendar-directory).
...
### Fixed
- **Duplicate event dropping most frontmatter on first try**: Duplicating an event could produce a copy missing dates, category, participants, and other fields until the action was repeated.
...
- **Batch Make Virtual / Make Real**: Convert multiple events between virtual and physical in batch selection mode. Select file-backed events and click "Make Virtual" to convert them all to virtual entries, or select manual virtual events and click "Make Real" to create backing files. Both buttons are available in the batch selection toolbar. See [Batch Operations](./features/management/batch-operations.md#make-virtual--make-real).
...
- **Weekdays and Weekends recurrence presets**: New "Weekdays (Mon–Fri)" and "Weekends (Sat–Sun)" options in the recurrence pattern dropdown. Selecting either preset automatically sets the correct days and locks the weekday checkboxes. Thanks to [@cweekly](https://github.com/cweekly) for the [idea](https://github.com/Real1tyy/Prisma-Calendar/issues/65). See [Recurring Events](./features/events/recurring-dsl.md).
...
### Fixed
- **Dragging a recurring instance to a new date deletes the event**: Moving a physical recurring instance by dragging it on the calendar caused the event to snap back to its original date and then disappear — the underlying file was deleted instead of moved. The Recurring Instance Date property is now immutable: it marks which occurrence slot the instance was generated for, while the display date (Date/Start Date) can be freely moved. See [Recurring Events](./features/events/recurring-dsl.md).
...
- **Sticky banner when toggling prerequisite connections**: Toggling prerequisite connection arrows now shows a bottom sticky banner indicating connections are enabled, with a Cancel button and Escape key to dismiss. See [Prerequisite Connections](./features/advanced/prerequisite-connections.md).

### Fixed
- **Prerequisite and status properties propagating to series and recurring instances**: Changing prerequisites or status on one event incorrectly propagated the change to name series, category series, and recurring instances. Both are per-event values and are now excluded from all propagation. See [Prerequisite Connections](./features/advanced/prerequisite-connections.md).
...
- **Tabbed views**: The calendar view now features a tab bar with five tabs — Calendar (original view), Timeline, Heatmap, Daily + Stats, and Dual Daily. Timeline and Heatmap are now persistent embedded views instead of modal overlays. Daily + Stats shows a daily calendar alongside synced statistics with pie chart. Dual Daily shows two independent daily calendars side-by-side. Toolbar buttons for Timeline and Heatmap now switch to the corresponding tab. Tab state (active tab, visibility, order) persists across sessions. Tabs can be reordered, renamed, and hidden via right-click or the settings gear. See [Tabbed Views](./features/views/tabbed-views.md).
