...
- **Undo / redo for category rename and delete**: Both operations are now part of the standard undo history. After a rename or delete, the success notice exposes an inline **Undo** link, and **Prisma Calendar: Undo** / **Prisma Calendar: Redo** from the command palette walk the file rewrites in one step. See [Categories → Renaming and Deleting Categories](./features/organization/categories.md#renaming-and-deleting-categories).
