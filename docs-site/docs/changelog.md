...
- **ICS export failing on events with reminders**: Exported ICS files with VALARM triggers used an invalid duration format or certain Rrule properties, causing re-import to fail with a parsing error. See [ICS Export](./features/integrations/ics-export.md).
