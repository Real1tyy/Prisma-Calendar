...
- **Prerequisite arrows on all-day events**: dragging an event onto a day that holds several all-day events no longer leaves its arrow pointing at whichever event sits at the top of that day's stack — the arrow now follows the moved event to its final spot in the lane, with no need to toggle the arrows off and on to fix it, and without the momentary flicker to the top event while the day settles. Arrows also stop guessing when there's nothing to point at: an event collapsed behind a **+N more** link keeps its arrow hidden until the event is visible again, and while you drag an event its arrow tracks the drag preview. An arrow between two all-day events is no longer hidden behind the sticky all-day row either. See [Prerequisite Connections](./features/advanced/prerequisite-connections.md#connection-arrows).
