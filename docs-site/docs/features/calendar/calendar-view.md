...
- **Settings and the event modal reflow to fit.** The color-rule, filter-preset, and category editors in **Settings** stack onto two rows at narrow width — the expression / name field on top, the color and controls below — so the inputs are wide enough to type in. The Create / Edit Event modal does the same: its checkboxes sit on one line with their label, and the date fields keep their picker readable next to compact **Now** / **Fill** buttons.
