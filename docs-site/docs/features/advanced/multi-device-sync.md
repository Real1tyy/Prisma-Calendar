...
| **CalDAV and ICS event notes** | The file name is derived from the remote event UID, so every device names the note the same way and sync never sees two copies. |
