...
- **Prerequisite property**: frontmatter property name for event prerequisites (default: `Prerequisite`). Stores wiki-links to other events that must complete before this event (e.g., `[[Team Meeting]], [[Design Review]]`). Shown as a text input in the Create/Edit Event modal when configured.
