...
describe("compareFrontmatter", () => {
	...
	it("should detect objects with same key count but different keys as modified", () => {
		const old: Frontmatter = { data: { a: 1, b: 2 } };
		const updated: Frontmatter = { data: { x: 1, y: 2 } };

		const diff = compareFrontmatter(old, updated);

		expect(diff.hasChanges).toBe(true);
		expect(diff.modified).toHaveLength(1);
		expect(diff.modified[0].key).toBe("data");
	});

	it("should detect objects with overlapping but different key sets as modified", () => {
		const old: Frontmatter = { data: { shared: 1, onlyOld: 2 } };
		const updated: Frontmatter = { data: { shared: 1, onlyNew: 2 } };
	...
	it("should detect different Date values as modified", () => {
		const diff = compareFrontmatter({ created: new Date("2026-01-01") }, { created: new Date("2026-12-31") });

		expect(diff.hasChanges).toBe(true);
		expect(diff.modified).toHaveLength(1);
		expect(diff.modified[0].key).toBe("created");
	});

	it("should treat identical Date values as unchanged", () => {
		const diff = compareFrontmatter({ created: new Date("2026-06-15") }, { created: new Date("2026-06-15") });
...
describe("mergeFrontmatterDiffs", () => {
	...
	it("should merge multiple diffs with mixed change types", () => {
		...
		const categoryChange = merged.added.find((c) => c.key === "category");
	...
	it("should preserve 'added' changeType when key is added then modified", () => {
		const diff1 = {
			hasChanges: true,
			changes: [{ key: "x", oldValue: undefined, newValue: 1, changeType: "added" as const }],
			added: [{ key: "x", oldValue: undefined, newValue: 1, changeType: "added" as const }],
			modified: [],
			deleted: [],
		};
		const diff2 = {
			hasChanges: true,
			changes: [{ key: "x", oldValue: 1, newValue: 2, changeType: "modified" as const }],
			added: [],
			modified: [{ key: "x", oldValue: 1, newValue: 2, changeType: "modified" as const }],
			deleted: [],
		};
		...
		expect(merged.added).toHaveLength(1);
		expect(merged.added[0].changeType).toBe("added");
		expect(merged.added[0].newValue).toBe(2);
		expect(merged.modified).toHaveLength(0);
	});
