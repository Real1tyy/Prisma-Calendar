...
5. **Token invalidation fallback** — match on two spec-grounded signals: the RFC 6578 §3.2 `<DAV:valid-sync-token/>` precondition element (surfaced in tsdav's error message) and an HTTP 410 status anchored behind a `status` / `HTTP` / `response` prefix so a bare `410` in a URL or timestamp doesn't false-trip. When the error object carries a structured `status` field we trust that directly instead of regexing the message. On match, clear the stored token and retry once with a full refetch passing **zero tombstones** (absence is still not a delete signal). Contract locked by `caldav-sync-token-invalidation.test.ts`, which covers both positive signals and the benign strings that tripped the previous catch-all.
